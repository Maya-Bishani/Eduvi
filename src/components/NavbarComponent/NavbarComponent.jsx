...
export const NavbarComponent = () => {
    ...
    return (
        <div className="container">
            <div className="nav">
                <div className="logo">
                    <img className="eduviLogo" src="/public/images/Navbar/Logo.svg" alt="EduviLogo" />
                </div>
                <GiHamburgerMenu  className="burger-icon"/>

                <div className="menu">
                    <ul>
                        {["Shop", "For Kindergarten", "For High School", "For College", "Courses"].map((item, index) => (
                            <li
                                key={index}
                                className="regular-font"
                                onClick={() => handleMenuClick(index)}
                            >
                                <Link className="tabs" to={`./${item.toLowerCase().replace(/ /g, "")}`}>
                                    {item} <FaAngleDown />
                                </Link>
